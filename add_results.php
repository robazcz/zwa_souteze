            ...
            if(!isset($_POST["selected-categories"])){
                ...
                foreach($cats as $cat){ 
                    ...
                    for($i=0; $i < count($_POST["results-name-$cat[id]"]); $i++){
                        echo "<li id='item-$cat[id]-$i'><div class='liflex'><div><label>Tým<span class='tooltip'>*<span class='tooltiptext'>Povinné pole</span></span>: <input type='text' list='list-$cat[id]-$i' name='results-name-$cat[id][]' value='{$_POST["results-name-$cat[id]"][$i]}' autocomplete='off' ". (in_array(strval($cat["id"]), $_POST["selected-categories"])?"required":"")." >";
                        echo "<datalist id='list-$cat[id]-$i'></datalist></label><p>".(isset($error["results-name"][$cat['id']][$i])?$error["results-name"][$cat['id']][$i]:"")."</p></div>";
                        echo "<div><label>Čas: <input type='number' step='.01' class='time' id='time-np-$cat[id]-$i' name='results-time1-$cat[id][]' value='{$_POST["results-time1-$cat[id]"][$i]}' placeholder='sekund'". (in_array(strval($cat["id"]), $_POST["selected-categories"])&&$_POST["results-np-$cat[id]"][$i]=="false"?"required":"")."></label><p>".(isset($error["results-time1"][$cat['id']][$i])?$error["results-time1"][$cat['id']][$i]:"")."</p></div>"; 
